...
// url 이름짓기
// 1. URL 명사로 작성 추천 -> /명사
// 2. 하위 문서를 나타낼 때 / slach (하위폴더 나누듯이 사용)
...
// _id 값에 totalDataLength에 1 증가한 값을 넣어주고
...
app.get('/info/:id', function(requests, response){
  ...
  // 'post' collection에 _id 값이 int이기 때문!
...
app.put('/edit', function(requests, response){
  db.collection('post').updateOne({_id : parseInt(parseInt(requests.body._id))}, {$set : {'아이디' : requests.body.id, '비밀번호' : requests.body.pw}}, function(error, result){
    console.log('수정 완료!')
...
// DB CRUD(Dreate, Read, Update, Delete)
